...
export const Primer2Card = ({selectedCell, selectedModel} : CardProps) => {
  ...
  const isHotNormal =
    ...
    outsoleTemp.value >= PM2_OT_TEMP_MIN &&
    outsoleTemp.value <= PM2_OT_TEMP_MAX;

  const isColdNormal =
    upperTemp?.value !== undefined &&
    upperTemp.value >= PM2_UP_TEMP_MIN &&
    upperTemp.value <= PM2_UP_TEMP_MAX;
        ...
        <div className="flex flex-wrap gap-4 items-end justify-between mt-5 md:px-2">
          <div>
            ...
            <div className="flex flex-col mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
              ...
              ({PM2_OT_TEMP_MIN}-{PM2_OT_TEMP_MAX}) °C
              </span>
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-col mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
              ...
              <span className="text-xs text-gray-400">
                ({PM2_UP_TEMP_MIN}-{PM2_UP_TEMP_MAX}) °C
              </span>
            </div>
          </div>
        </div>
