// ./src/components/charts/line/ChartCementing.tsx
"use client";
...
const CELL_MAP: Record<string, CellConfig> = {
  "B1-01": { plcId: "2", upper: "data8", outsole: "data9" },
  "B1-02": { plcId: "5", upper: "data8", outsole: "data9" },
};

const STANDARDS_BY_MODEL: Record<string, MachineStandardLimits> = STANDARDS;
...
export const ChartCementing = ({selectedCell, selectedModel, title}: CardProps) => {
  ...
  const createAnnotations = useCallback((): Record<string, LineAnnotation> => { 
      ...
      annotations['hotMax'] = createLineAnnotation(standards.CM_OT_TEMP_MAX, 'Heating Standard', hotColor, 'end', 'MAX');
      annotations['hotMin'] = createLineAnnotation(standards.CM_OT_TEMP_MIN, 'Heating Standard', hotColor, 'start', 'MIN');
      
      const coldColor = 'rgb(0, 0, 255)'; 
      annotations['coldMax'] = createLineAnnotation(standards.CM_UP_TEMP_MAX, 'Molding Standard', coldColor, 'end', 'MAX');
      annotations['coldMin'] = createLineAnnotation(standards.CM_UP_TEMP_MIN, 'Molding Standard', coldColor, 'start', 'MIN');
  ...
  useEffect(() => {
    ...
    const pollApi = async () => {
      try {
        ...
        setDataHistory((prev) =>
          processData(apiResponse, prev, tagsToDisplay, filterRange)
        );
        setLoading(false);
      } catch (err) {
  ...
  const exportChartAsImage = (format: "png" | "jpeg") => {
    ...
    if (!chartInstance) return alert("Grafik belum siap.");
    const canvas = chartInstance.canvas;
    const mimeType = format === "png" ? "image/png" : "image/jpeg";
    const link = document.createElement("a");
    link.href = canvas.toDataURL(mimeType);
    link.download = `grafik_${new Date().toISOString()}.${format}`;
    link.click();
  };
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] flex flex-col justify-between col-span-12 lg:col-span-4">
      ...
      <div style={{ width: "100%", height: "400px" }}>
        {!hasData && !loading ? (
          <p style={{ color: "#aaa", textAlign: "center", marginTop: "100px" }}>
            Memuat data...
          </p>
        ) : (
