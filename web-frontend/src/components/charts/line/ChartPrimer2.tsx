...
const TAG_LABELS: { [key: string]: string } = {
    "data6": "Upper Temp", "data7": "Outsole Temp", 
};
...
export const ChartPrimer2 = ({ selectedCell, selectedModel, title }: CardProps) => {
 ...
   
    ...
    annotations['uperMax'] = createLineAnnotation(standards.PM2_OT_TEMP_MAX, 'Outsole Std', hotColor, 'end', 'MAX');
    annotations['upperMin'] = createLineAnnotation(standards.PM2_OT_TEMP_MIN, 'Outsole Std', hotColor, 'start', 'MIN');
    
    const coldColor = 'rgb(0, 0, 255)'; 
    annotations['outsoleMax'] = createLineAnnotation(standards.PM2_UP_TEMP_MAX, 'Upper Std', coldColor, 'end', 'MAX');
    annotations['outsoleMins'] = createLineAnnotation(standards.PM2_UP_TEMP_MIN, 'Upper Std', coldColor, 'start', 'MIN');
