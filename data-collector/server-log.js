...
const { saveHistoricalData, saveHistoricalDataLog } = require("./database/db-client");
...
const { historyProcessorLoopLog } = require("./alarm/getPlcfromLog");
...
async function pollingLoop() {
  try {
    ...
    if (allData.length > 0) {
      try {
        await saveHistoricalDataLog(allData);
      } catch (dbErr) {
